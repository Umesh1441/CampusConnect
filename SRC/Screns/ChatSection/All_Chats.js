...
import ChatsCards from '../../Carts/ChatsCards';
...
const All_Chats = ({ navigation }) => {
    ...
    //     }
    // ]
    const filterUniqueRecieverId = (messages) => {
        const uniqueRecieverIds = new Set();
      
        // Filter the messages array
        const uniqueMessages = messages.filter((message) => {
          const recieverId = message.recieverid;
      
          // If the recieverId is not present in the Set, add it and return true
          if (!uniqueRecieverIds.has(recieverId)) {
            uniqueRecieverIds.add(recieverId);
            return true;
          }
      
          // If recieverId is already present, return false
          return false;
        });
    ...
    const loadchats = () => {
        AsyncStorage.getItem('user')
            .then(data => {
                ...
                fetch('http://192.168.43.24:3000/getusermessages', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        // console.log(data)
                        const newdata=filterUniqueRecieverId(data);
                        // console.log("now:");
                        // console.log(newdata);
                        newdata.sort((a, b) => {
                            if(a.date > b.date){
                    ...
                    .catch(err => {
                        alert('Something went wrong at meaage')
                        setChats([])
                    })
            })
            .catch(err => alert(err))
            ...
            <View style={styles.c2}>
                {
                    chats!==null && chats.filter(
                        ...
                        // console.log(chat);
                        return <ChatsCards key={chat.fuserid} chat={chat.recieverid} navigation={navigation}/>
                    })
                }
            </View>
        </ScrollView>
